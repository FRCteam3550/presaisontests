...
public class Auto3BallesCommandPropre extends SequentialCommandGroup {
  /** Creates a new Auto1BalleEtReculCommand. */
  public Auto3BallesCommandPropre(DriveTrain drivetrain, Grimpeur grimpeur, SuperSubsystem convoyeur, Shooter shooter, SuperSubsystem ramasseur,PhotoSwitches photoSwitches, String[] path_to_run) {
    // Add your commands in the addCommands() call, e.g.
    // addCommands(new FooCommand(), new BarCommand());
    addCommands(
    new ConvoyorUPCommand(convoyeur, photoSwitches),
    parallel(new RunPathCommand(drivetrain, path_to_run), new RamasseurExtendCommand(ramasseur)), //RunPathCommand: reculer
    new RamasseurRamasseCommand(ramasseur, photoSwitches),
    new RunPathCommand(drivetrain, path_to_run),
    new ShooterONcommand(shooter),
    new ConvoyorUPCommand(convoyeur, photoSwitches),
    new ShooterONcommand(shooter),
    parallel(new ShooterONcommand(shooter), new RunPathCommand(drivetrain, path_to_run)), //RunPathCommand: avancer
