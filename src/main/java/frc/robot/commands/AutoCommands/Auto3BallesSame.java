...
public class Auto3BallesSame extends SequentialCommandGroup {
  /** Creates a new Auto1BalleEtRecul2Command. */
  public Auto3BallesSame(DriveTrain drivetrain, Grimpeur grimpeur, SuperSubsystem convoyeur, Shooter shooter,
      SuperSubsystem ramasseur, PhotoSwitches photoSwitches, String[] path_to_run1, String[] path_to_run2, String[] path_to_run3, String[] path_to_run4) {
   ...
   //Prepare le shooter au tire, tire avec convoyeurUp et extension du ramasseur
       ...
       //Activation du moteur du ramasseur et parcour de la premiere partie de la trajectoire
            parallel(//new RamasseurDebugCommand(ramasseur).withTimeout(0.5),
                    new RunPathCommand(drivetrain, path_to_run1).withTimeout(2.5)), //3.5
    
                   

         //Ramassage de la balle, parcour de la deuxieme partie de la trajectoire et positionnement de la balle dans le convoyeur
            parallel(new RunPathCommand(drivetrain, path_to_run2).withTimeout(2.0), //3.0
         ...
         //Tire de la deuxieme balle
            ...
            new ShootStopper(convoyeur, ramasseur, shooter, photoSwitches).withTimeout(0.25),
            
            parallel(new RunPathCommand(drivetrain, path_to_run3).withTimeout(3.5), //3.0
                    new RamasseurAutoCommand2(ramasseur, photoSwitches).andThen(new ConvoyorUPAutoIndividualEnSerieCommand(convoyeur, photoSwitches).withTimeout(0.25))),
            new ConvoyorSTOPCommand(convoyeur).withTimeout(0.25),

            new ShooterONAutoPracticecommand(shooter).withTimeout(0.5),

            parallel(new RunPathCommand(drivetrain, path_to_run4).withTimeout(2.0),
            new ConvoyorUP2IndividualCommand(convoyeur, photoSwitches).withTimeout(1.0)),

                     //Tire de la troisieme balle
                     new AutoShoot2Bottom(convoyeur, ramasseur, shooter, photoSwitches).withTimeout(1.0),
                     new ShootStopper(convoyeur, ramasseur, shooter, photoSwitches).withTimeout(0.25),
        ...
        //  new RunPathCommand(drivetrain, path_to_run1).withTimeout(3.0);
